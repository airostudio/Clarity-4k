...
function ClientCard({ client: c }: { client: ClientRow }) {
  return (
    <Link href={`/clients/${c.id}`} className="card hover:border-brand-600/40 transition-colors group block">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-slate-400 truncate">{c.industry || 'No industry set'}</div>
          </div>
        </div>
      ...
      {c.contactName && (
        <p className="text-xs text-slate-400 mt-3">{c.contactName}{c.contactEmail ? ` · ${c.contactEmail}` : ''}</p>
      )}
      ...
      <div className="grid grid-cols-2 gap-2 mt-3">
        ...
        <div className="bg-surface rounded-lg px-3 py-2 text-center">
          ...
          <div className="text-xs text-slate-500">{c.activeDeals} active</div>
        </div>
      </div>

      {c.website && (
        <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-surface-border text-xs text-slate-400 truncate">
